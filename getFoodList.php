...
<div id="allFoodsContainer" class="allFoodsContainer">
    ...
    // Fetch food list with status for the logged-in user and calculate average rating from comments
    $sql = "SELECT food_list.*, location.canteenName AS canteenName, 
            IF(student_favorite.userName IS NULL, 0, 1) AS status,
            IFNULL(AVG(comments.rating), 0) AS averageRating
            FROM food_list
            ...
            LEFT JOIN comments ON food_list.foodId = comments.foodId
            GROUP BY food_list.foodId";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div class='food-rate'>" . number_format($row['averageRating'], 1) . "分</div>";
            echo "<div class='food-price'>￥" . $row['foodPrice'] . "</div>";
            echo "<button onclick='showOrderPopup(" . $row['foodId'] . ", \"" . $row['foodName'] . "\", \"" . $row['foodImage'] . "\", \"" . number_format($row['averageRating'], 1) . "\", " . $row['foodPrice'] . ", \"" . $row['canteenName'] . "\", \"" . $row['canteenId'] . "\")'>+</button>";
            echo "</div>";
            echo "</div>";
        }
    } else {
